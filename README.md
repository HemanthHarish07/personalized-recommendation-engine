...
This project implements a **real-world personalized recommendation system** using classical collaborative filtering techniques, with a strong emphasis on **baselines, honest offline evaluation, and system-level clarity** rather than metric chasing.
...
Modern digital platforms rely heavily on recommender systems to surface relevant items to users. A naive approach that recommends globally popular items often fails to capture individual user preferences, leading to reduced engagement and poor user experience.
...
- Evaluating all models using a **consistent ranking-based metric (Hit Rate@K)**  
...
- **Popularity definition:** Interaction count  
- **Purpose:** Sanity check and cold-start fallback  
- **Limitation:** Ignores individual user preferences  
...
- **Core assumption:** Users with similar past behavior will have similar future preferences  
...
- User-based CF improves relevance but does not scale well  
...
- **Evaluation:** Custom offline ranking metrics (Hit Rate@K)  
...
Baselines are critical for honest recommender system evaluation

Personalization must be justified relative to non-personalized approaches

Item-based collaborative filtering offers a strong balance between performance and scalability

Offline ranking metrics require careful interpretation under sparse data

Clean system design and reproducibility matter as much as model choice
